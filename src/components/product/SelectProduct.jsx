...
import { addToFavorites, availableNotification, deleteFavorites } from "@/services/ProductPage";
...
const SelectProduct = ({ data }) => {
  ...
  const [selectedColor, setSelectedColor] = useState(data.product_color[0].color.id);
  ...
  const incCount = () => {
    if (count < data.product_color.filter((color) => color.color.id === selectedColor)[0].quantity)
      setCount(count + 1);
  };
  ...
  const fetchData = async () => {
    const favResponse = await fetch(`${baseUrl}/api/product/${data.slug}/fav/`, {
      method: "GET",
      headers: {
  ...
  return (
    <>
      <Box display="flex" maxWidth={400} justifyContent="space-between" mt={2} ml={3}>
        <Box>
          <Box display="flex" alignContent="center">
            ...
            <Typography
              ...
              {(data.product_color.length > 0 &&
                data.product_color.filter((color) => color.color.id === selectedColor)[0].color.name) ||
                "انتخاب نشده"}
            </Typography>
          </Box>
        ...
        <Box>
          <Typography
            ...
            {`تعداد: (موجودی ${
              data.product_color.filter((color) => color.color.id === selectedColor)[0].quantity
            })`}
          </Typography>
          <Box
            display="flex"
            ...
            <IconButton
              onClick={incCount}
              disabled={
                data.product_color.filter((color) => color.color.id === selectedColor)[0].quantity <= count
              }>
              <Box
                display="flex"
                alignItems="center"
      ...
      <Box mt={2} mx={3} width="90%" border="0.1em solid #22668D" />
      <Box
        mt={3}
        ml={3}
        ...
        {!data.is_available ? (
          ...
          <Box>
            <Typography noWrap mt={0.5} align="center" sx={{ fontSize: { xs: 18, md: 20 } }} fontWeight={400}>
